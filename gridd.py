...
def compare(last_board, current_board):
    flag = True
    m = len(last_board)
    n = len(last_board[0])
    for i in range(0, m):
        for j in range(0, n):
            if last_board[i][j]["value"]!=current_board[i][j]["value"] or last_board[i][j]["dir"]!=current_board[i][j]["dir"]:
                flag = False
    return flag
...
def main():
    ...
    for i in range(0, len(last_board)):
        for j in range(0, len(last_board[0])):
            if last_board[i][j]["value"]==1 and last_board[i][j]["dir"]=="final":
                row = i
                col = j
    ...
    rowrow = int(start_x)
    colcol = int(start_y)
    while (last_board[rowrow][colcol]["value"]==1 and last_board[rowrow][colcol]["dir"]=="final")==False:
